...
TestController.prototype.createSkeleton = function(x, y, width, height) {
	...
	*/
	...
	var //neckLastLink = neck[neck.length-1],
		...
		torso = this.createBody({x: headCenter.x, y: headCenter.y + head.radius * 1.5 /*+ neckLinkHeight/2*/ + torsoHeight / 2, width: torsoWidth, height: torsoHeight, restitution: restitution});
	...
	this.createRevoluteJoint(head, torso, null, -15/180 * Math.PI, 15/180 * Math.PI);
	...
	this.createRevoluteJoint(torso, leftHumerus, new Box2D.Common.Math.b2Vec2(torsoLeft, torsoTop), -85/180*Math.PI, 175/180*Math.PI);
	...
	this.createRevoluteJoint(torso, rightHumerus, new Box2D.Common.Math.b2Vec2(torsoRight, torsoTop), -175/180*Math.PI, 85/180*Math.PI);
