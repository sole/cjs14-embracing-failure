function TestController(intervalRate, adaptive, context, debugView, scale, gravityX, gravityY) {
	...
	this.lastCreatedPumpkinTime = 0;
	
	// 'world bounds'
	var w2 = this.width * 0.5, h2 = this.height * 0.5, thickness = 40;

	//var boundTop = this.createBody({x: w2/scale, y: -thickness/scale, width: (this.width*1.2)/scale, height: thickness/scale, type: Body.TYPE_STATIC}); // Top	
	var boundBottom = this.createBody({x: w2/scale, y: (thickness/2+this.height)/scale, width: (this.width*1.2)/scale, height: thickness/scale, type: Body.TYPE_STATIC}); // Bottom
	var boundLeft = this.createBody({x: -thickness/2/scale, y: h2/scale, width: thickness/scale, height: this.height*10.2/scale, type: Body.TYPE_STATIC}); // Left
	var boundRight = this.createBody({x: (this.width+thickness/2)/scale, y: h2/scale, width: thickness/scale, height: this.height*10.2/scale, type: Body.TYPE_STATIC}); // Right
	
	var x = w2 / scale, y = -40/scale, numSkeletons = 0;
			
	for(var i = 0; i < 15; i++) {
		var itemWidth, itemHeight;
		
		x = this.width/2 / scale; //(Math.random() * this.width - thickness) / scale;
		
		if(Math.random() > 0.5 && numSkeletons < 2) {
			itemWidth = Math.max(0.5, Math.random()) * 140 / scale;
			...
			if(Math.random() > 0.5) {
				...
				itemWidth = Math.max(0.5, Math.random()) * 25 / scale;
				itemHeight = itemWidth; 
				console.log('calabaza', x, y);
				var p = this.createPumpkin(
					x, y, itemWidth, itemHeight
				);
				p.boxBody.SetAngularVelocity(10 * (Math.random() - 0.5));
			} else {
				itemHeight = Math.max(0.5, Math.random()) * 100 / scale;
				itemWidth = itemHeight/10; 
				console.log('hueso', x, y);
				var p = this.createBone(
					x, y, itemWidth, itemHeight
				);
				p.boxBody.SetAngularVelocity(10 * (Math.random() - 0.5));
			}
	...
	// Add skeleton(s)
	/*var prevSk = null, numSkeletons = 2, offset = this.width/(numSkeletons+1);
	for(var i = 0; i < numSkeletons; i++) {
		var size = Math.max(0.5, Math.random()) * 120 / scale;
		//var sk = this.createSkeleton(
		...
		//this.createDistanceJoint(boundTop, sk.head, new Box2D.Common.Math.b2Vec2(skHeadCenter.x, 0), skHeadCenter, 1, 0.1);
		
		//sk.torso.boxBody.ApplyForce(new Box2D.Common.Math.b2Vec2(4000 * (Math.random() - 0.5), 4000 * (Math.random() - 0.5)), sk.torso.boxBody.GetWorldCenter());
		 //sk.torso.boxBody.SetAngularVelocity(200 * (Math.random() - 0.5));
...
TestController.prototype.createBone = function(x, y, width, height) {
	var body = this.createBody({ x: x, y: y, width: width, height: height, shape: Body.SHAPE_RECTANGLE, restitution: 0.0 });
	
	this.useSVGForBody(body, 'bone.svg');
	
	return body;
}

TestController.prototype.createPumpkin = function(x, y, width, height) {
	var pumpkin = this.createBody({ x: x, y: y, radius: width, shape: Body.SHAPE_CIRCLE, restitution: 0.1 });
	
	this.useSVGForBody(pumpkin, 'pumpkin.svg');
...
TestController.prototype.createSkeleton = function(x, y, width, height) {
	...
	var headCenter = head.boxBody.GetWorldCenter(),
		...
		neck = this.makeChain(
			...
			neckLinkHeight * 1.5, neckLinkHeight * 1, // link width & height
			neckLinkHeight * 0, // link separation,
			true, // vertical direction?
			neckNumLinks, // number of links
			restitution,
			-15/180 * Math.PI,
			15/180 * Math.PI
		);
	...
	var	torsoLeft = x - torsoWidth / 2,
		...
		armWidth = width / 16,
		fingerWidth = armWidth / 3,
		handMult = 1.5,
		littleFingerHeight = height / 18 * handMult,
		ringFingerHeight = height / 15 * handMult,
		midFingerHeight = height / 13 * handMult,
		indexFingerHeight = height / 16 * handMult,
		thumbFingerHeight = height / 30 * handMult;
	...
	this.createRevoluteJoint(torso, leftHumerus, new Box2D.Common.Math.b2Vec2(torsoLeft, torsoTop), -255/180*Math.PI, 225/180*Math.PI);
	...
	this.createRevoluteJoint(torso, rightHumerus, new Box2D.Common.Math.b2Vec2(torsoRight, torsoTop), -225/180*Math.PI, 255/180*Math.PI);
	...
	// Setting SVGs for rendering
	this.useSVGForBody(head, 'skull.svg');
	this.useSVGForBody([leftHumerus, rightHumerus, leftFemur, rightFemur], 'bone.svg');
	this.useSVGForBody([leftRadius, rightRadius, leftTibia, rightTibia], 'double_bone.svg');
	this.useSVGForBody(pelvis, 'pelvis.svg');
	this.useSVGForBody(torso, 'torso.svg');
	this.useSVGForBody([leftThumbFinger, rightThumbFinger], 'two_bone_finger.svg');
	this.useSVGForBody([leftLittleFinger, leftRingFinger, leftMidFinger, leftIndexFinger, rightIndexFinger, rightMidFinger, rightRingFinger, rightLittleFinger], 'three_bone_finger.svg');
...
TestController.prototype.useSVGForBody = function(body, svgFile) {
	...
	function renderBitmapRectangle(context) {
	...
	function renderBitmapCircle(context) {
		...
		context.drawImage(canvas, (this.x - this.radius)* this.scale, (this.y - this.radius) * this.scale, this.radius*2*this.scale, this.radius*2*this.scale);
		
		context.restore();
	}
	
	function getRenderFunction(body) {
		return (body instanceof BodyCircle ? renderBitmapCircle : renderBitmapRectangle);
	}
	
	console.log(this.svgCanvasCache, this.svgCanvasCache[svgFile]);

	if(typeof this.svgCanvasCache[svgFile] === 'undefined') {
		console.log("No se ha cargado aun svgFile", svgFile);
		canvas = document.createElement('canvas');
		this.svgCanvasCache[svgFile] = canvas;
		canvg(canvas, svgFile, {
			ignoreMouse: true,
			ignoreAnimation: true,
			offsetX: 0,
			offsetY: 0,
			scaleWidth: (body instanceof BodyCircle ? body.radius*2 : body.width) * body.scale,
			scaleHeight: (body instanceof BodyCircle ? body.radius*2 : body.height) * body.scale,
			renderCallback: function() {
				
				
					if(body instanceof Array) {
						for(var i = 0; i < body.length; i++) {
							body[i].drawingCallback = getRenderFunction(body);
						}
					} else {
						body.drawingCallback = getRenderFunction(body);
					}
				}
				
		});
	} else {
		console.log("reutilizar", svgFile);
		canvas = this.svgCanvasCache[svgFile];
	...
	/*	//if(this.svgCanvasCache[svgFile] !== 'undefined') {
